...
from langchain_groq import ChatGroq  # Assuming Groq model usage
...
class EvaluationSchema(BaseModel):
    ...
    score: Optional[int] = Field(default=None, description="Score out of 10", ge=0, le=10)

structured_model = model.with_structured_output(EvaluationSchema)

def extract_score(out: EvaluationSchema) -> int:
    if out.score is not None:
        return out.score
    # Fallback: extract from feedback
    match = re.search(r"Score:?\s*(\d+)", out.feedback, re.IGNORECASE)
    if match:
...
def evaluate_language(state: UPSCState):
    prompt = (
        ...
        "Provide detailed feedback and a separate integer score (0-10).\n\n" + state["essay"]
    )
    out = structured_model.invoke(prompt)
    return {"language_feedback": out.feedback, "individual_scores": [extract_score(out)]}
...
def evaluate_analysis(state: UPSCState):
    ...
    out = structured_model.invoke(prompt)
    return {"analysis_feedback": out.feedback, "individual_scores": [extract_score(out)]}
...
def evaluate_thought(state: UPSCState):
    ...
    out = structured_model.invoke(prompt)
    return {"clarity_feedback": out.feedback, "individual_scores": [extract_score(out)]}
